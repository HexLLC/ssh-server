...
RUN mkdir /run/sshd \
    ...
    && echo "curl -s http://localhost:4040/api/tunnels | python3 -c \"import sys, json; tunnels = json.load(sys.stdin)['tunnels']; public_url = tunnels[0]['public_url'][6:] if tunnels else ''; print('SSH info:\\nssh root@' + public_url.replace(':', ' -p ') + '\\nROOT Password: craxid') if public_url else print('Ngrok tunnels not found.')\" || echo 'Error: NGROK_TOKEN not set or Ngrok issue'" >> /openssh.sh \
